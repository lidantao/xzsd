...
public class OrderService {
    ...
    public Response addOrder(OrderDTO orderDTO) {
        ...
        //查看销量，卖出去要加销量
        List<String> goodsSallNumList = orderDao.finGoodsSallNum(orderDTO);
        ...
        for (int i = 0; i < goodsStockList.size(); i++) {
            ...
            //销量增加
            int goodsSallNum = Integer.parseInt(goodsSallNumList.get(i)) + Integer.parseInt(orderDTO.getGoodsCountList().get(i));
        ...
        if (orderDTO.getShopCarCodeList() != null) {
    ...
    public Response addGoodsEvaluate(JSONObject obj) {
        ...
        if (add1 > 0) {
            ...
            for (EvaluateInfo evaluateInfo1 : avgScore) {
                ...
                int avgStar = Math.toIntExact(Math.round(evaluateInfo1.getAvgScore()));
                map.put("avgScore", avgStar);
                map.put("lastUpdateUser", SecurityUtils.getCurrentUserId());
                avgStarList.add(map);
            }
            //更新商品评价等级
