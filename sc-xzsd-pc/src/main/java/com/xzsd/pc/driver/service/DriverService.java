...
//        if(driverInfo.getRole().equals(ROLE)){
//            //登录角色为店长，查询自己门店下的司机信息
//            List<DriverInfo> list = driverDao.listDriverByStoreByPage(driverInfo);
//        }
    ...
    public Response updateDriver(DriverInfo driverInfo) {
        ...
        //密码校验过程
        //获取旧的存在数据库的司机信息
        DriverInfo driverInfo1 = driverDao.findDriver(driverInfo.getDriverCode());
        //获取原密码（上一次修改或添加的已加密的原密码）
        String oldPassord = driverInfo1.getUserPassword();
        //获取原密码（本次修改密码传的原密码是未加密的哦）
        String newPassword = driverInfo.getRawUserPassword();
        //比较密码是否相同
