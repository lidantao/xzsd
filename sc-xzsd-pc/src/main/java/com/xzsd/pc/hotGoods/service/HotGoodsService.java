...
public class HotGoodsService {
    ...
    public Response listHotGoods(HotGoodsInfo hotGoodsInfo) {
        ...
        if (hotGoodsInfoList.size() == 0) {
            ...
            //获取分页列表数量
            int size = hotGoodsInfoList.size();
            //展示数量叠加
            for (int i = 1; i < hotGoodsInfo1.getHotGoodsShowNum(); i++) {
                for (int j = 0; j < size; j++) {
                    hotGoodsInfoList.add(hotGoodsInfoList.get(j));
                }
            }
            return Response.success("查询成功！", getPageInfo(hotGoodsInfoList));
        }
    }
