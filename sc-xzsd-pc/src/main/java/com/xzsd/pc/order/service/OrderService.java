...
public class OrderService {
    ...
    public Response updateOrderActive(OrderDTO orderDTO) {
        ...
        //设置修改成什么订单状态
        orderInfo.setOrderActive(orderDTO.getOrderActive());
        //查看需要修改订单编号数量
        int length = orderDTO.getOrderCodeList().size();
        for (int i = 0; i < length; i++) {
            Map map = new HashMap();
            map.put("oderCode",orderDTO.getOrderCodeList().get(i));
            map.put("version",orderDTO.getVersionList().get(i));
            orderInfo.getMapList().add(map);
        }
